"""
Resume Setup 2 from where it died.
Existing: DB1 ✅, DB2 ✅, DB3 partial (done through f=28,k=7).
Remaining: DB3 f=28 k={9,11}, f=30 k={5,7,9,11}, then all of DB4.
Appends to the existing CSV.
"""
import sys
import csv
import json
...
REMAINING = [
    # DB3 remainder
    ("fvc2002_3", 28, [9, 11]),
    ("fvc2002_3", 30, [5, 7, 9, 11]),
    # All of DB4
    ("fvc2004_1", 14, [5, 7, 9, 11]),
    ("fvc2004_1", 16, [5, 7, 9, 11]),
    ("fvc2004_1", 18, [5, 7, 9, 11]),
    ("fvc2004_1", 20, [5, 7, 9, 11]),
    ("fvc2004_1", 22, [5, 7, 9, 11]),
...
def main():
    ...
    for db_name, factor, degrees in REMAINING:
        if db_name not in loaded_dbs:
            print(f"\n{'='*80}")
            print(f"DATABASE: {db_name} | SETUP 2 (RESUME)")
            print(f"{'='*80}")
            loaded_dbs[db_name] = load_chimeric_data(db_name, min_iris=8)
            subjs = loaded_dbs[db_name]
            print(f"Loaded {len(subjs)} subjects, test imps: {get_test_impressions(subjs)}")

        subjects = loaded_dbs[db_name]
        n_subj = len(subjects)
    ...
    print(f"\n{'='*80}")
    print(f"RESUME COMPLETE — {total:.0f}s ({total/3600:.1f}h)")
